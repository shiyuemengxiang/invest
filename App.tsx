  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans">
      ...
      <div className={`${themeConfig.sidebar} md:hidden p-4 flex justify-between items-center sticky top-0 z-50 shadow-md`}>
        ...
        <button onClick={() => setView('add')} className={`${themeConfig.button} text-white px-3 py-1 rounded-md text-sm`}>
            + 记一笔
        </button>
      </div>
      ...
      <div className={`hidden md:flex flex-col w-64 ${themeConfig.sidebar} h-screen sticky top-0 transition-colors duration-300`}>
        ...
        <nav className="flex-1 px-4 space-y-2">
            <button 
                ...
                className={`w-full text-left px-4 py-3 rounded-xl transition flex items-center gap-3 ${view === 'dashboard' ? 'bg-white/10 text-white font-bold' : 'hover:bg-white/5 opacity-80'}`}
            >
            ...
            <button 
                ...
                className={`w-full text-left px-4 py-3 rounded-xl transition flex items-center gap-3 ${view === 'list' ? 'bg-white/10 text-white font-bold' : 'hover:bg-white/5 opacity-80'}`}
            >
            ...
            <button 
                ...
                className={`w-full text-left px-4 py-3 rounded-xl transition flex items-center gap-3 ${view === 'calendar' ? 'bg-white/10 text-white font-bold' : 'hover:bg-white/5 opacity-80'}`}
            >
            ...
            <button 
                ...
                className={`w-full text-left px-4 py-3 rounded-xl transition flex items-center gap-3 ${view === 'profile' ? 'bg-white/10 text-white font-bold' : 'hover:bg-white/5 opacity-80'}`}
            >
        ...
        <div className="p-4">
            <button 
                ...
                className={`w-full py-3 rounded-xl font-bold shadow-lg transition flex justify-center items-center gap-2 bg-white text-slate-900 hover:bg-slate-100 active:scale-95`}
            >
            ...
            {!user && (
                <button 
                    ...
                    className="w-full mt-3 text-xs text-center text-white/50 hover:text-white py-2"
                >
                    登录 / 注册同步
                </button>
            )}
